...
impl WakfloAuthApi {
    pub fn new(base_url: String, client: ureq::Agent) -> WakfloAuthApi {
        ...
        WakfloAuthApi { base_url, client, ory_conf: conf }
    }
    ...
    pub(crate) fn login(
        ...
        let local_var_uri_str = format!("{}/self-service/login/api?return_session_token_exchange_code=true", &self.ory_conf.base_path);
        let flow_rsp = self.client.get(local_var_uri_str.as_str()).call()?;
        let flow = flow_rsp.into_json::<CustomFlow>()?;

        let rsp = ory_client::apis::frontend_api::update_login_flow(&self.ory_conf, flow.id.clone().as_str(), &ory_client::models::UpdateLoginFlowBody::UpdateLoginFlowWithPasswordMethod {
            csrf_token: None,
            identifier: identifier.clone(),
            password: password.clone(),
            password_identifier: None,
        }, None, None).wait().expect("");
    ...
    pub(crate) fn whoami(&self) -> anyhow::Result<WakfloUser> {
        ...
        match req.call() {
            ...
            Err(e) => {
                anyhow::bail!(format!("{}", e.to_string().replace("https://api.wakflo.io/v1/auth/whoami:", "")))
            }
        }
    }
}
